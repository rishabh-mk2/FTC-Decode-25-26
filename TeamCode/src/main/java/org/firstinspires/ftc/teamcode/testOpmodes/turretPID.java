...
public class turretPID extends OpMode {
    ...
    public void loop() {
        ...
        if (!result.isValid()) {
            ...
            turretShooter.getMotor(TurretShooter.MotorNames.turret).setPower(0);
            return;
        }
        ...
        double turretPos = turretShooter.getMotor(TurretShooter.MotorNames.turret).getCurrentPosition();

        double targetTicks = turretPos - (384.5 * currentTargetDeg) / 360.0;

        double pid = controller.calculate(turretPos, targetTicks);

        turretShooter.getMotor(TurretShooter.MotorNames.turret).setPower(pid + f*targetTicks);
